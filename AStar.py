...
def AStarAlgorithm(start_node, end_node, grid, enc):
    ...
    while open_list:
        ...
        for neighbor in neighborNodes(current.grid_index, grid, enc, current):
            if neighbor not in closed_list:
                neighbor.f = neighbor.g + heuristic(neighbor, end_node)
...
def draw_line_path(start_point, end_point, path, enc, color):
    ...
    for node in path_copy:
        ...
        #print("node index: ", node.grid_index, "node h: ", node.f - node.g)
        point_list.append(waypoint)
...
def neighborNodes(node_index, node_grid, enc, current_node):
    #print("Finding valid neighbors...")
    neighbors = []
    radial_cost = 1  # cost of moving radially
    diagonal_cost = 1.1  # cost of moving diagonally
    ...
    try:
        ...
        print("Nort-East is outside the grid!")
    ...
    for neighbor in neighbors:
        ...
        if neighbor is not current_node.parent:  # no cycles
            ...
            if neighbor is north_node or neighbor is west_node or neighbor is east_node or neighbor is south_node:
                neighbor.g = current_node.g + radial_cost  # update cost
            else:
                neighbor.g = current_node.g + diagonal_cost  # update cost
